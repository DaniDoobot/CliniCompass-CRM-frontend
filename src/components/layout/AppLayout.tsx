...
export function AppLayout({ children, consoleMode = false }: { children: React.ReactNode; consoleMode?: boolean }) {
  return (
    <SidebarProvider>
      <div className={consoleMode ? "h-screen flex w-full overflow-hidden" : "min-h-screen flex w-full"}>
        <AppSidebar />
        <div className={consoleMode ? "flex-1 flex flex-col min-w-0 overflow-hidden" : "flex-1 flex flex-col min-w-0"}>
          ...
          <main className={consoleMode ? "flex-1 flex flex-col overflow-hidden" : "flex-1 p-6 overflow-auto"}>
            {children}
          </main>
